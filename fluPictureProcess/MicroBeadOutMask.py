...
class MicroBeadOutMask():
    ...
    def calc_microbead_mask(self):
        self.outline = 4
        self.mask_width = round(self.center_dist * (self.std_cols+self.outline+0.5))
        self.mask_height = round(self.row_height * (self.std_rows+self.outline))
        np_img_mask = np.zeros([self.mask_height, self.mask_width], dtype='uint8')
        cent = np.zeros([self.std_rows+self.outline, self.std_cols+self.outline, 2],dtype='float32')
        # 处理W
        for h_i in range(self.std_rows+self.outline):
            h_pos = (h_i+0.5)*self.row_height
            for w_i in range(self.std_cols+self.outline):
                w_pos = 0
                if h_i%2 == 0:
                    w_pos = (w_i*2 + 2)*self.center_dist/2.0
                else:
                    w_pos = (w_i*2 + 1) * self.center_dist / 2.0
        ...
        img_show("np_img_mask",np_img_mask)
        exit()
        pass
...
def run_flu_signal_regnize(center_dist, split_dir, out_dir):
    w_h = 46
    mbm = MicroBeadOutMask(center_dist)
    ori_signal_sets = []
    en_signal_sets = []
    for i in range(w_h):
        ...
        for j in range(w_h):
            ...
            # 自动识别参数，并进行荧光信息加强
            blacks,whites,gammas = MicroBeadOutMask.calc_auto_BGR_cutoff(img)
    ...
    MicroBeadOutMask.store_flu_signal_2_file(outfile=outfile, flu_signal=ori_signal_sets)
    outfile = f"{out_dir}/signal_en.txt"
    MicroBeadOutMask.store_flu_signal_2_file(outfile=outfile, flu_signal=en_signal_sets)
